...
The axis labels may or may not be relevant for the dimensions. Sometimes I
swept the Lithium Niobate deposition thickness, and sometimes I fixed the
thickness but swept the etch-depth """
...
def eps_func_sbstrt(freq):
    assert isinstance(freq, float) or isinstance(freq, int) or \
           all([isinstance(freq, np.ndarray), len(freq.shape) ==
                1, freq.shape[0] > 1])  # 1D array with length > 1
...
def eps_func_wvgd(omega): return Gayer5PctSellmeier(
    24.5).n((1 / omega) * 1e3) ** 2
...
def get_disp(n): return np.load(path_disp + name_disp[n])


def plot_nu_eps(n): return plt.plot(
    get_disp(n)[:, 1], (get_disp(n)[:, 0] / get_disp(n)[:, 1]) ** 2, '.-')


def plot_wl_eps(n): return plt.plot(
...
def get_field(n): return np.squeeze(np.load(path_fields + name_fields[n]))
...
def get_eps(n): return np.load(path_eps + name_eps[n])


def plot_eps(n): return plt.imshow(
...
def plot_mode(n, k_index, new_figure=True):
    ...
    if guided:
        ...
        s = "NOT guided"
    plt.title(f'{np.round(width(name_eps[n]), 3)} x '
              f'{np.round(height(name_eps[n]), 3)}' +
              ' $\\mathrm{\\mu m}$' '\n' +
              "$\\mathrm{\\lambda = }$" + '%.2f' % wl +
              ' $\\mathrm{\\mu m}$' + '\n' +
...
def check_if_guided(n): return is_guided(get_disp(n)[:, 0], freq)
...
#     ax.clear()
#     ax.plot(1 / freq, i, 'o-')
#     ax.set_title(str(n) + " | " + str(width(name_disp[n])) + " x "
#                  + str(height(name_disp[n])))
#     ax.axhline(0, color='k', linestyle='--')
#     ax.axvline(1.55, color='k', linestyle='--')
#     plt.pause(1)
#
...
#         ax.clear()
#         ax.plot(BETA2[n], 'o-')
#         ax.set_title(f'{np.round(width(name_disp[n]), 2)} x '
