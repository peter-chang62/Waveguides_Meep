...
def plot_mode(n, k_index, new_figure=True, ax=None):
    ...
    wl = 1 / freq[k_index]
    if ax is not None:
        ax.set_title(f'{np.round(width(names[n]), 3)} x {np.round(depth(names[n]), 3)} x 1' + ' $\mathrm{\mu m}$' '\n' +
                     "$\mathrm{\lambda = }$" + '%.2f' % wl + ' $\mathrm{\mu m}$' + '\n' +
                     '$\mathrm{A_{eff}}$ = %.3f' % mode_area(get_field(n)[k_index]) +
                     ' $\mathrm{\mu m^2}$')
    else:
        plt.title(f'{np.round(width(names[n]), 3)} x {np.round(depth(names[n]), 3)} x 1' + ' $\mathrm{\mu m}$' '\n' +
                  "$\mathrm{\lambda = }$" + '%.2f' % wl + ' $\mathrm{\mu m}$' + '\n' +
                  '$\mathrm{A_{eff}}$ = %.3f' % mode_area(get_field(n)[k_index]) +
                  ' $\mathrm{\mu m^2}$')
...
#     a_eff = mode_area(get_field(ind)[ind_center_wl]) * 1e-12  # um^2 -> m^2 @ lamda = 1560 nm
#
...
#     [i.clear() for i in ax]
#
#     p_v_dB = 10 * np.log10(data)
#     ax[0].pcolormesh(wl_grid, z, p_v_dB, vmin=-40, vmax=10)
